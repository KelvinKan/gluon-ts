...
class LowRankMultivariateNormalOutput(DistributionOutput):
    ...
    def __init__(
        ...
        assert (
                sigma_init >= 0
        ), "sigma_minimum should be greater than or equal to 0"

        assert (
            sigma_minimum > 0
        ), "sigma_minimum should be greater than 0"
        ...
        if rank == 0:
            ...
            self.args_dim = {"mu": target_dim, "D": target_dim, "W": target_dim * rank}
    ...
    # Let's try if domain_map can be non-classmethod
    def domain_map(
            self,
        ...
        # D_bias = (
        #     torch.log(torch.exp(self.sigma_init * torch.ones(1,
        ...
        if self.rank == 0:
            W_matrix = torch.zeros((*mu_vector[:-1], self.target_dim, 1),
                                   dtype=mu_vector.dtype,
                                   device=mu_vector.device,
                                   layout=mu_vector.layout,
                                   )
            ...
            W_matrix = W_vector.reshape(*W_vector.shape[:-1], self.target_dim, self.rank)
    ...
    def distribution(
        ...
        if loc==0 and scale is None:
            ...
            # TODO check if mean and std are correct
...
sto = LowRankMultivariateNormalOutput(2,2)
st = sto.distribution(sto.domain_map(mu,d,w))
