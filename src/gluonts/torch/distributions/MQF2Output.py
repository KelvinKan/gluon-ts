...
class MQF2Distribution(Distribution):
    ...
    def unfold_observations(self, z: torch.Tensor) -> torch.Tensor:
        ...
        Then, reshapes the observations into a 2-dimensional tensor
    ...
    def energy_score(self, z: torch.Tensor) -> torch.Tensor:
        ...
        loss = picnn.energy_score(z, hidden_state, es_num_samples=es_num_samples, beta=beta)
    ...
    def rsample(self, sample_shape=torch.Size()) -> torch.Tensor:
        ...
        -------
        sample_paths
            tesnor of shape (batch_size, num_samples_per_batch, prediction_length)
        """
        ...
        num_samples_per_batch = self.get_numel(sample_shape)
        num_samples = num_samples_per_batch * numel_batch

        hidden_state_repeat = hidden_state.repeat_interleave(repeats=num_samples_per_batch, dim=0)
        ...
        return self.quantile(alpha, hidden_state_repeat).reshape(numel_batch, num_samples_per_batch, prediction_length)
...
class MQF2DistributionOutput(DistributionOutput):
    ...
    def distribution(
        ...
        if scale is None:
            return self.distr_cls(picnn, hidden_state, prediction_length=self.prediction_length, threshold_input=self.threshold_input, es_num_samples=self.es_num_samples, is_energy_score=self.is_energy_score,
                                  beta=self.beta,)
        else:
            distr = self.distr_cls(picnn, hidden_state, prediction_length=self.prediction_length, threshold_input=self.threshold_input, es_num_samples=self.es_num_samples, is_energy_score=self.is_energy_score,
                                   beta=self.beta,)
