...
class MQF2MultiHorizonModel(DeepARModel):
    ...
    def __init__(
        ...
        convexnet = ConvexNet(dim=prediction_length, dimh=icnn_hidden_size, dimc=hidden_size,
                     num_hidden_layers=icnn_num_layers, symm_act_first=True,
                     softplus_type='gaussian_softplus',
                     zero_softplus=True)
        deepconvexnet = DeepConvexNet(convexnet, prediction_length, is_energy_score=is_energy_score, estimate_logdet=estimate_logdet)

        if is_energy_score:
            networks = [deepconvexnet]
        else:
            networks = [ActNorm(prediction_length),
                      deepconvexnet,
    ...
    def unroll_lagged_rnn(
        ...
        Tuple[SequentialNet, torch.Tensor],
        torch.Tensor,
    ]:
        """
    ...
    def output_distribution(
            self, picnn, hidden_state, scale=None, inference=False,
        ...
        inference
            If True, pass only the last hidden state to the forecastor for prediction
            Otherwise, pass all the hidden states to train the forecastor
