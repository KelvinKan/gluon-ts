...
from icnn_lib.icnn import PICNN, PICNN_SN
from gluonts.torch.distributions.CPFlow import CPFlowDistributionOutput
...
class MultivariateDeepARModel(nn.Module):
    @validated()
    def __init__(
        self,
        freq: str,
        ...
        if use_flow_architecture:
            icnn = PICNN_SN(self.prediction_length, icnn_hidden_size, hidden_size, icnn_num_layers, symm_act_first=True,
                            softplus_type='gaussian_softplus',
                            zero_softplus=True)
            layers = [None] * 3
            layers[0::2] = [ActNorm(self.prediction_length) for _ in range(2)]
            layers[1] = DeepConvexFlow(icnn, self.prediction_length, unbiased=False, atol=cg_atol)
        else:
            icnn = PICNN(self.prediction_length, icnn_hidden_size, hidden_size, icnn_num_layers, symm_act_first=True,
                            softplus_type='gaussian_softplus',
                            zero_softplus=True)
            layers = [DeepConvexFlow(icnn, self.prediction_length, unbiased=False, no_identity_term=True, atol=cg_atol)]
    ...
    def unroll_lagged_rnn(
        ...
        # params = (self.flow, output[:, -1, :]) # This is for the last window prediction only
        params = (self.flow, output)
    ...
    def output_distribution(
        ...
        sliced_params = params
        if trailing_n is not None:
            sliced_params = (params[0],) + tuple(p[:, -1] for p in params[1:])

        sliced_params = sliced_params + (self.threshold_input, self.num_expected)
