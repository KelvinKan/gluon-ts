...
class DeepVARModel(nn.Module):
    ...
    def __init__(
        ...
        self.lagged_rnn = LaggedLSTM(
            # This should probably be changed to target_dim
    ...
    def _number_of_features(self) -> int:
        # newly added here
        return self.num_feat_dynamic_real
    ...
    def unroll_lagged_rnn(
        ...
        past_time_feat = (batch_size, past_length, num_time_feat)
        past_target = (batch_size, past_length, target_dim)
        past_observed_values = (batch_size, past_length, target_dim)
        ...
        # #newly added reshape here
        # # embedded_cat = (batch, 50) -> (batch, 50, target_dim)
        # # feat_static_real = (batch, 1) -> (batch, 1, target_dim)
        # # scale = (batch, 1, target_dim)
        # embedded_cat = embedded_cat.unsqueeze(-1).expand(-1,-1,self.target_dim)
        # feat_static_real = feat_static_real.unsqueeze(-1).expand(-1,-1,self.target_dim)
        ...
        #newly added here
        # time_feat = time_feat.unsqueeze(-1).expand(-1, -1, -1, self.target_dim)

        #newly added here
        # features = torch.cat((expanded_static_feat, time_feat), dim=-2)

        # TODO: add target_dimension_indicator to this
        # or maybe to the forward of the LSTM
        features = time_feat
        ...
        #newly added here
        return params, scale, output, new_state
    ...
    def forward(
        ...
        # Return: (batch_size, num_parallel_samples, prediction_length, target_dim)
        
        if num_parallel_samples is None:
            num_parallel_samples = self.num_parallel_samples

        params, scale, _, state = self.unroll_lagged_rnn(
            feat_static_cat,
            feat_static_real,
            past_time_feat,
            past_target,
...
class LaggedLSTM(nn.Module):
    def __init__(
        ...
        #newly added here
        self.target_dim = target_dim
        self.rnn_input_size = input_size * len(self.lags_seq) * self.target_dim + features_size
    ...
    def get_lagged_subsequences(
        ...
        for lag_index in indices:
            ...
            lagged_values.append(sequence[:, begin_index:end_index, ...].unsqueeze(dim=1))
        # newly added
        aggregate_data = torch.cat(lagged_values, dim=1)
        return aggregate_data.permute(dims=(0, 2, 3, 1))
    ...
    def forward(
        ...
        # Let's deal with the shape check later
        ...
        # Add embedding target_dimension_indicator here
